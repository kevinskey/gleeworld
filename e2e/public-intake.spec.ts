// End-to-end spec for public tenant intake — anonymous appointment booking and
// audition submission (Task 11 of the public-tenant-intake plan).
//
...
test.describe('public intake — no login wall', () => {
  test.beforeEach(async ({ page }) => {
    await asStranger(page);
  });

  test('/book renders for a visitor with no session', async ({ page }) => {
    await page.goto('/book');
    // Assert on the URL, not page text: the regression is a redirect to
    // sign-in, and a styling change must not be able to mask it.
    await expect(page).not.toHaveURL(/\/auth/);
    await expect(page).toHaveURL(/\/book/);
    await expect(
      page.getByRole('heading', { name: /book|appointment/i }).first(),
    ).toBeVisible();
  });

  test('/book-appointment redirects to /book, not to sign-in', async ({ page }) => {
    // Every appointment-booking block saved before this feature still points at
    // the old path. This guards those without the tenant re-editing anything.
