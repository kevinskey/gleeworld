...
export function buildSystemPrompt(ctx: AssistantContext): string {
  ...
  const platformNote = [
    ...
    '- Notes, shown as "Notes" in the nav and also known as the Planner (notes, tasks, to-dos): private notes and tasks — where create_note saves. Users may say "Notes" or "Planner" for the same app — treat them as identical and NEVER correct the user about its name; answer using whichever name the user used.',
  ...
  const pagesNote = [
    ...
    '- "Open my X class" / "take me to the Y course": that is an Academy COURSE, not a page — call list_courses to find its id, then open_course. Only fall back to open_page academy when no course matches.',
