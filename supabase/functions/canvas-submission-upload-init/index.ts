// canvas-submission-upload-init — step 1 of Canvas's 3-step file
// upload. The browser uploads the file *directly* to Canvas's signed
// URL (S3 or local), then calls canvas-submit-assignment to finalize
// the submission with the returned file_id.
//
// Body: { course_id, assignment_id, name, size, content_type? }
...
serve(async (req) => {
  ...
  let body: { course_id?: number; assignment_id?: number; name?: string; size?: number; content_type?: string };
  try { body = await req.json(); } catch { return err(400, "bad_json"); }
  if (!body.course_id || !body.assignment_id) return err(400, "missing_ids");
  if (!body.name || !body.size) return err(400, "missing_file_meta");
  ...
  try {
    const init = await resolved.client.requestSubmissionUpload({
