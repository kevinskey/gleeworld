// canvas-get-submission — fetch a single student's submission with
// attachments + comments + rubric assessment. Used by SpeedGrader-lite.
//
// Body: { course_id, assignment_id, user_id }

import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import { createClient } from "jsr:@supabase/supabase-js@2";
import { getCanvasClientForTenant } from "../_shared/canvas.ts";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
};

function decodeJwtPayload(jwt: string): Record<string, unknown> | null {
  try {
    const part = jwt.split(".")[1];
    const padded = part + "===".slice((part.length + 3) % 4);
    return JSON.parse(atob(padded.replace(/-/g, "+").replace(/_/g, "/")));
  } catch { return null; }
}

function err(status: number, code: string, detail?: string) {
  return new Response(JSON.stringify({ error: code, detail }), {
    status, headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}

serve(async (req) => {
  if (req.method === "OPTIONS") return new Response(null, { headers: corsHeaders });
  if (req.method !== "POST") return err(405, "method_not_allowed");

  const jwt = (req.headers.get("Authorization") || "").replace(/^Bearer\s+/i, "");
  if (!jwt) return err(401, "unauthorized");
  const payload = decodeJwtPayload(jwt);
  // deno-lint-ignore no-explicit-any
  const tenantId = (payload as any)?.tenant_id ?? (payload as any)?.app_metadata?.tenant_id;
  // deno-lint-ignore no-explicit-any
  const userId = (payload as any)?.sub;
  if (!tenantId || !userId) return err(400, "no_tenant_or_user_in_jwt");

  let body: { course_id?: number; assignment_id?: number; user_id?: number };
  try { body = await req.json(); } catch { return err(400, "bad_json"); }
  if (!body.course_id || !body.assignment_id || !body.user_id) return err(400, "missing_ids");

  const admin = createClient(
    Deno.env.get("SUPABASE_URL") ?? "",
    Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "",
    { auth: { autoRefreshToken: false, persistSession: false } },
  );

  const resolved = await getCanvasClientForTenant(admin, tenantId);
  if (!resolved) return err(409, "canvas_not_bound");

  const { data: profile } = await admin
    .from("gw_profiles").select("canvas_user_id").eq("user_id", userId).maybeSingle();
  if (!profile?.canvas_user_id) return err(409, "canvas_user_not_provisioned");

  try {
    const s = await resolved.client.getSubmission({
      courseId: body.course_id, assignmentId: body.assignment_id,
      userId: body.user_id, asUserId: profile.canvas_user_id,
    });
    return new Response(JSON.stringify({
      ok: true,
      submission: {
        id: s.id,
        user_id: s.user_id,
        user_name: s.user?.name ?? `User ${s.user_id}`,
        user_avatar: s.user?.avatar_url ?? null,
        score: s.score,
        grade: s.grade,
        submitted_at: s.submitted_at,
        late: s.late,
        missing: s.missing,
        workflow_state: s.workflow_state,
        submission_type: s.submission_type ?? null,
        body: s.body ?? null,
        url: s.url ?? null,
        attachments: (s.attachments ?? []).map((a) => ({
          id: a.id, name: a.display_name, url: a.url,
          content_type: a["content-type"], size: a.size,
        })),
        comments: (s.submission_comments ?? []).map((c) => ({
          id: c.id, body: c.comment, author: c.author_name, at: c.created_at,
        })),
        rubric_assessment: s.rubric_assessment ?? null,
      },
    }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    return err(502, "canvas_request_failed", e instanceof Error ? e.message : String(e));
  }
});
