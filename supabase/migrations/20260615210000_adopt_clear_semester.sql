-- adopt_course_template: clear semester + term on adopt.
--
-- Templates use semester='TEMPLATE' and term=NULL as a marker, but those
-- were getting copied verbatim into the adopted course, leaving the new
-- class showing "TEMPLATE" as if that were a real semester label. The
-- instructor should set semester/term themselves once they adopt.
...
BEGIN
  ...
  --   - is_template flipped off (new course is a real class, not a template).
  --   - semester/term cleared so the instructor sets them.
  --   - instructor reset to the caller.
  INSERT INTO gw_courses (
    ...
    NULL, NULL,            -- term, semester cleared on adopt
    v_user_id,
...
-- Backfill: clear the leftover "TEMPLATE" semester on any rows that were
-- adopted before this fix. Only touches adopted (non-template) rows whose
-- semester is the marker string.
UPDATE public.gw_courses
   SET semester = NULL, term = NULL
 WHERE is_template = false
   AND semester = 'TEMPLATE';
