-- Switch to URL-safe base64 encoding (uses - and _ instead of + and /)
...
BEGIN
  ...
  -- Generate URL-safe token: replace + with -, / with _, remove trailing =
  v_qr_token := replace(replace(encode(extensions.gen_random_bytes(32), 'base64'), '+', '-'), '/', '_');
