-- Fix the process_qr_attendance_scan function to read from the correct table (gw_attendance_qr_codes)
-- The generate_session_qr_code function writes to gw_attendance_qr_codes but scan was reading from gw_attendance_qr_tokens

CREATE OR REPLACE FUNCTION public.process_qr_attendance_scan(p_qr_token text, p_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Look in gw_attendance_qr_codes table (where generate_session_qr_code writes)
  SELECT 
    id,
    qr_token,
    attendance_session_id as session_id,
    expires_at,
    is_active as is_used_inverted -- Note: is_active=true means NOT used
  INTO v_token_record
  ...
  -- Get session details from gw_attendance_sessions
