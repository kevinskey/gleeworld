...
USING (EXISTS (
  SELECT 1 FROM gw_profiles 
  WHERE user_id = auth.uid() 
  AND (is_super_admin = true OR exec_board_role = 'secretary' OR special_roles ? 'secretary')
));
...
-- Admins and section leaders can review excuse requests
...
USING (EXISTS (
  SELECT 1 FROM gw_profiles 
  WHERE user_id = auth.uid() 
  AND (is_admin = true OR is_super_admin = true OR special_roles ? 'section_leader')
));
