...
-- 3. Fix audition_analytics - Add RLS policies for sensitive PII data
CREATE POLICY "Admins can view audition analytics" 
ON audition_analytics 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 FROM gw_profiles 
...
CREATE POLICY "Admins can manage audition analytics" 
ON audition_analytics 
FOR ALL 
USING (
  EXISTS (
...
-- 4. Add unique constraint for payment integrity
ALTER TABLE gw_user_orders 
