-- v_student_ledger: unified view of money owed/paid across 2 source adapters.
...
-- finance_records is deliberately NOT adapted. It is an organizational
-- stipend/reimbursement ledger (money the org owes a person), not student
-- debt: stipends are written with POSITIVE amounts and type 'stipend', and
-- nothing ever settles a row — status would derive from `type`, which never
-- changes, so a charge would stay outstanding forever. Including it made
-- sp_balance report a $250 stipend PAID TO a student as $250 they OWE.
...
-- Dropped after v_student_ledger is redefined so the dependency is gone first.
drop view if exists student_picture.led_finance;
