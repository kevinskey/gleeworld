-- Fix Community Hub permissions - Final corrected version
...
TO authenticated
USING (
  -- Users can view their own requests OR view anonymous requests OR approved ones
  (auth.uid() = user_id) OR 
...
-- 3. Fix gw_communications: Allow authenticated users to view communications (using sender_id)
...
USING (
  ...
  (status = 'published') OR
  -- Senders, admins, and exec board can view all
...
-- 4. Allow authenticated users to create communications
