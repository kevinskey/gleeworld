...
-- Add optional provider_id to gw_appointments for tracking which provider handled the appointment
ALTER TABLE public.gw_appointments 
ADD COLUMN provider_id UUID REFERENCES public.gw_service_providers(id);

-- Update existing appointments to link to Dr. Johnson if service matches
UPDATE public.gw_appointments 
SET provider_id = (
  SELECT id FROM public.gw_service_providers 
  WHERE provider_name = 'Dr. Kevin Phillip Johnson' 
  LIMIT 1
)
