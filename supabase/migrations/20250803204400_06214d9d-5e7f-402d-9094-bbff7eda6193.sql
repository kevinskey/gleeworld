...
TO authenticated
USING (
  -- Users can view their own requests OR view anonymous/shared requests
  (auth.uid() = user_id) OR 
  (is_anonymous = true) OR
  (is_shared_with_community = true) OR
...
TO authenticated
USING (
  -- Anyone can view if shared to members
  (is_shared_to_members = true) OR
  -- Chaplains and admins can view all
  (EXISTS (
    SELECT 1 FROM gw_profiles 
    WHERE user_id = auth.uid() 
