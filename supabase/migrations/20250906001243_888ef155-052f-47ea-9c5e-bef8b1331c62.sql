-- Fix recursive trigger issue by dropping the problematic triggers
-- and recreating them with proper recursion prevention

-- Drop the existing triggers that are causing the recursion
...
-- Create improved sync functions with recursion prevention
...
BEGIN
  ...
  -- Insert or update wardrobe profile (with conflict handling to prevent recursion)
  INSERT INTO public.gw_member_wardrobe_profiles (
    user_id, measurements, formal_dress_size, polo_size, 
    tshirt_size, lipstick_shade, pearl_status, updated_at
  ) VALUES (
    NEW.user_id,
    NEW.measurements,
  ...
  WHERE 
    ...
    (gw_member_wardrobe_profiles.measurements IS DISTINCT FROM EXCLUDED.measurements OR
...
BEGIN
  ...
  -- Update the corresponding gw_profiles record (with conditions to prevent recursion)
  ...
  AND (
    -- Only update if values are actually different to prevent recursion
    measurements IS DISTINCT FROM NEW.measurements OR
