...
BEGIN
  ...
  -- Extract duration from special_requests if it mentions a specific duration
  -- Parse "General Meeting (30 min)" or similar from special_requests
  actual_duration := service_record.duration_minutes;
  IF p_special_requests IS NOT NULL THEN
    IF p_special_requests ILIKE '%30 min%' THEN
      actual_duration := 30;
    ELSIF p_special_requests ILIKE '%15 min%' THEN
      actual_duration := 15;
    ELSIF p_special_requests ILIKE '%45 min%' THEN
      actual_duration := 45;
    ELSIF p_special_requests ILIKE '%60 min%' THEN
      actual_duration := 60;
    END IF;
  END IF;
  
  availability_check := public.check_appointment_availability(
    p_service_id, p_appointment_date, p_start_time, actual_duration
  );
