...
DROP POLICY IF EXISTS "Users can view their own orders" ON public.gw_user_orders;
DROP POLICY IF EXISTS "Users can create orders" ON public.gw_user_orders;
DROP POLICY IF EXISTS "Users can update their own orders" ON public.gw_user_orders;

CREATE POLICY "Users can view their own orders or guest orders" ON public.gw_user_orders
  FOR SELECT USING (
    user_id = auth.uid() 
...
CREATE POLICY "Anyone can create orders" ON public.gw_user_orders
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Users can update their own orders" ON public.gw_user_orders
  FOR UPDATE USING (
    user_id = auth.uid() 
    OR (user_id IS NULL AND auth.uid() IS NULL)
    OR EXISTS (
...
-- Update products table RLS to allow public viewing
DROP POLICY IF EXISTS "Everyone can view products" ON public.gw_products;
CREATE POLICY "Everyone can view active products" ON public.gw_products
  FOR SELECT USING (is_active = true);

-- Update product variants RLS
DROP POLICY IF EXISTS "Everyone can view variants" ON public.gw_product_variants;
CREATE POLICY "Everyone can view product variants" ON public.gw_product_variants
  FOR SELECT USING (true);

-- Update payment records RLS for guest checkout
DROP POLICY IF EXISTS "Users can view their payment records" ON public.gw_payment_records;
CREATE POLICY "Users can view their payment records" ON public.gw_payment_records
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.gw_user_orders o 
      WHERE o.id = gw_payment_records.order_id 
      AND (o.user_id = auth.uid() OR auth.uid() IS NULL)
    )
  );

-- Insert some sample products for the store
INSERT INTO public.gw_products (title, description, price, product_type, vendor, tags, inventory_quantity, images, is_active) VALUES
