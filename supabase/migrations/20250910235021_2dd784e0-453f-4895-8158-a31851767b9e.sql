-- Update the trigger function to use assignment_db_id instead of assignment_id
...
DECLARE
  existing_id uuid;
BEGIN
  -- Skip if assignment_db_id is null (assignment not mapped)
  IF NEW.assignment_db_id IS NULL THEN
    RETURN NEW;
  END IF;
  ...
  WHERE student_id = NEW.student_id
    AND assignment_id = NEW.assignment_db_id
  ...
  IF existing_id IS NULL THEN
    INSERT INTO public.assignment_submissions (
      ...
      NEW.student_id, NEW.assignment_db_id, 'graded', now(), now(),
