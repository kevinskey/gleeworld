CREATE OR REPLACE FUNCTION public.process_qr_attendance_scan(p_qr_token text, p_user_id uuid, p_user_agent text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  IF v_qr_record.attendance_session_id IS NOT NULL THEN
    ...
    -- Fixed: removed non-existent qr_scan_id column
  ...
  -- BRANCH 2: Event-based (gw_events only)
