-- Public intake: anonymous audition submission.
--
...
COMMENT ON COLUMN public.gw_branding_settings.welcome_sms_template IS
  'Welcome SMS sent after a public audition submission. '
  'Placeholders: {org_name}, {first_name}. NULL falls back to '
  '"Thanks for joining {org_name}!".';

-- NOTE: this migration originally also granted anon EXECUTE on
-- get_available_time_slots, for a /book page that has since been dropped —
-- main's appointment-booking block completes a booking inline via
-- gw_booking_public_submit. Neither that grant nor any book_appointment change
-- belongs here; auditions do not touch either function.
