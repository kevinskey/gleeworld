-- Create Study Scores tables and sharing function, plus storage bucket
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column_v2'
  ) THEN
...
-- Owner can do anything on their study scores
...
-- Collaborators can view study scores shared with them
...
-- Owner can manage collaborator rows for their study scores
...
-- 5) Storage bucket for study scores (public for simplicity)
...
DECLARE
  ...
  v_user_id uuid;
BEGIN
  -- Validate caller owns the study score
  ...
  -- Find the user by email in gw_profiles
  ...
  -- Grant annotate permission on the derived sheet music
  BEGIN
...
-- 7) Annotations: allow collaborators to read and insert for study scores
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema='public' AND table_name='gw_sheet_music_annotations'
  ) THEN
    -- SELECT policy for collaborators and owners
    ...
    -- INSERT policy for collaborators and owners
