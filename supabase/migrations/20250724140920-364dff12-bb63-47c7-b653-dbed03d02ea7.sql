-- PHASE 1: CRITICAL SECURITY FIXES (Corrected column names)

-- 1. Enable RLS on gw_appointments
DO $$
BEGIN
...
-- 2. Drop existing policies on key tables and recreate securely
DO $$
DECLARE
    policy_name text;
BEGIN
    -- Drop gw_profiles policies
    FOR policy_name IN 
...
-- 4. Add RLS policy for gw_appointments (using correct column names)
...
-- 6. Add secure storage policies (drop existing first)
DROP POLICY IF EXISTS "Admins can access all performer documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can access their own files in user-files bucket" ON storage.objects;
...
-- 8. Create security logging function
