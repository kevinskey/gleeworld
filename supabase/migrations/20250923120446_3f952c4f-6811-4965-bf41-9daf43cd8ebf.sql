...
BEGIN
  ...
  current_date := parent_event.start_date;
  ...
  WHILE occurrence_count < max_count LOOP
    ...
    CASE recurrence_type_param
      WHEN 'daily' THEN
        current_date := current_date + (recurrence_interval_param || ' days')::interval;
      WHEN 'weekly' THEN
        current_date := current_date + (recurrence_interval_param * 7 || ' days')::interval;
      WHEN 'monthly' THEN
        current_date := current_date + (recurrence_interval_param || ' months')::interval;
      ELSE
        RAISE EXCEPTION 'Invalid recurrence type: %', recurrence_type_param;
    END CASE;
    
    -- Check if we've exceeded the end date
    IF recurrence_end_date_param IS NOT NULL AND current_date > recurrence_end_date_param THEN
      EXIT;
    END IF;
    ...
    IF recurrence_type_param = 'weekly' AND recurrence_days_of_week_param IS NOT NULL THEN
      IF NOT (EXTRACT(DOW FROM current_date)::integer = ANY(recurrence_days_of_week_param)) THEN
        CONTINUE;
      END IF;
    END IF;
    
    -- Calculate end date for this occurrence
    end_date := current_date + duration_interval;
    ...
    INSERT INTO public.gw_events (
      ...
      current_date, end_date, parent_event.location, parent_event.venue_name,
      ...
      CASE 
        WHEN parent_event.attendance_deadline IS NOT NULL THEN 
          current_date + (parent_event.attendance_deadline - parent_event.start_date)
        ELSE NULL 
      END,
