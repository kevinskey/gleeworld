-- First, let's add the assistant_chaplain to the executive_position enum
ALTER TYPE public.executive_position ADD VALUE 'assistant_chaplain';
...
-- Create RLS policies for chaplain and admin access (using just 'chaplain' since assistant_chaplain is now added to enum)
...
FOR ALL USING (
  EXISTS (
    ...
    AND position IN ('chaplain'::executive_position, 'assistant_chaplain'::executive_position)
    AND is_active = true
  ) OR EXISTS (
