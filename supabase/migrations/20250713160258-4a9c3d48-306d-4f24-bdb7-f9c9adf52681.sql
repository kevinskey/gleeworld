-- Phase 1: Attendance System Database Schema (Corrected)

-- Add voice_part to gw_profiles table if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'gw_profiles' 
                 AND column_name = 'voice_part') THEN
    ALTER TABLE gw_profiles 
    ADD COLUMN voice_part text CHECK (voice_part IN ('soprano_1', 'soprano_2', 'alto_1', 'alto_2', 'tenor_1', 'tenor_2', 'bass_1', 'bass_2'));
  END IF;
END $$;
...
CREATE POLICY "Secretaries and section leaders can take attendance" 
ON gw_event_attendance 
FOR INSERT 
WITH CHECK (
  is_admin(auth.uid()) OR 
