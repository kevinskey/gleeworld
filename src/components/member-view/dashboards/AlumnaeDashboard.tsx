...
export const AlumnaeDashboard = ({ user }: AlumnaeDashboardProps) => {
  ...
  useEffect(() => {
    const fetchAlumnaeData = async () => {
      try {
        // Fetch upcoming events marked as alumnae-related
        const { data: alumnaeEvents } = await supabase
          .from('gw_events')
          .select('id, title, start_date, location')
          .gte('start_date', new Date().toISOString())
          .or('title.ilike.%reunion%,title.ilike.%alumni%,title.ilike.%alumnae%')
        ...
        setAlumnaeData(prev => ({
          ...prev,
          upcomingAlumnaeEvents: alumnaeEvents?.map(event => ({
            id: event.id,
            title: event.title,
        ...
        console.error('Error fetching alumnae data:', error);
      }
    };
