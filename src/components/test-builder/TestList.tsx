...
export const TestList = ({
  ...
  return <>
      <div className="space-y-2 sm:space-y-3">
        {tests.map(test => <div key={test.id} className="group relative flex flex-col lg:flex-row lg:items-center justify-between p-3 sm:p-4 md:p-5 border rounded-lg sm:rounded-xl hover:shadow-md transition-all duration-200 bg-card hover:border-primary/30">
            <div className="flex-1 space-y-1.5 sm:space-y-2 mb-3 lg:mb-0 lg:mr-4">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4 text-[10px] sm:text-xs text-muted-foreground">
                ...
                {test.duration_minutes && <span className="flex items-center gap-1 text-primary-foreground">
                    <span className="font-medium text-primary-foreground">{test.duration_minutes}</span> min
                  </span>}
                <span className="flex items-center gap-1 text-primary-foreground">
                  Pass: <span className="font-medium text-primary-foreground">{test.passing_score}%</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {test.id === 'original-midterm' ? <>
                  ...
                  <Button variant="default" size="sm" onClick={() => navigate(`/test/${test.id}/preview`)} className="relative overflow-hidden bg-primary hover:bg-primary/90 shadow-sm hover:shadow-md transition-all group/preview text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">
                  ...
                  {test.is_published && <>
                      <Button variant="default" size="sm" onClick={() => navigate(`/test/${test.id}/take`)} className="bg-green-600 hover:bg-green-700 text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">
                        <PlayCircle className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
                        Take Test
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => navigate(`/test/${test.id}/scores`)} className="hover:bg-accent hover:border-primary/30 text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">
                        <BarChart3 className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
                        <span className="hidden xs:inline">View </span>Scores
                      </Button>
                    </>}
                  <Button variant="outline" size="sm" onClick={() => setTestToDelete(test.id)} className="hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30 transition-all text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">
