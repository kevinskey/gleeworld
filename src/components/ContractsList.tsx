...
export const ContractsList = ({
  ...
  const {
    contractSendHistory,
    reloadSendHistory
  } = useContractSendHistory(contracts);
  const {
    lastRecipient,
    loading: lastRecipientLoading
  } = useLastRecipient(sendDialogContract?.id || null);
  const {
    signingContract,
    adminSignature,
    showSignatureModal,
  ...
  return <>
      ...
      {sendDialogContract && <SendContractDialog contract={sendDialogContract} isOpen={!!sendDialogContract} onClose={handleSendDialogClose} onSent={handleContractSent} initialRecipientEmail={isResendMode && lastRecipient && !lastRecipientLoading ? lastRecipient.recipientEmail : ""} initialRecipientName={isResendMode && lastRecipient && !lastRecipientLoading ? lastRecipient.recipientName : ""} isResend={isResendMode} lastRecipientLoading={lastRecipientLoading} />}

      {/* Admin Signature Modal */}
      <AdminSignatureModal isOpen={showSignatureModal} contract={contractToSign} adminSignature={adminSignature} isSigningContract={signingContract === contractToSign?.id} onSignatureChange={setAdminSignature} onClose={closeSignatureModal} onComplete={handleCompleteAdminSigning} />
