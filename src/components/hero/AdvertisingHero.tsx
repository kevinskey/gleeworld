...
export const AdvertisingHero: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="w-full aspect-[16/9] sm:aspect-[21/9] md:aspect-[2/1] bg-muted animate-pulse rounded-xl" />
    );
  }
  ...
  const content = (
    <div 
      className="relative w-full aspect-[16/9] sm:aspect-[21/9] md:aspect-[2/1] rounded-xl overflow-hidden group shadow-lg"
    ...
    >
      ...
      {heroes.map((h, index) => (
        ...
        >
          {/* Desktop Image */}
          <img
            src={h.image_url || fallbackImage}
            alt={h.title || 'Featured promotion'}
          ...
          {/* iPad/Tablet Image */}
          <img
            src={h.ipad_image_url || h.image_url || fallbackImage}
            alt={h.title || 'Featured promotion'}
          ...
          {/* Mobile Image */}
          <img
            src={h.mobile_image_url || h.image_url || fallbackImage}
            alt={h.title || 'Featured promotion'}
      ...
      {(hero.title || hero.description) && (
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8 text-white z-30 pointer-events-none">
          ...
          {hero.description && (
            <p className="text-sm sm:text-base md:text-lg text-white/90 max-w-2xl drop-shadow line-clamp-2">
              {hero.description}
            </p>
          )}
        </div>
      ...
      {/* Link Indicator */}
      {hero.link_url && (
        <div className="absolute top-4 right-4 bg-white/20 backdrop-blur-sm rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity z-30">
      ...
      {heroes.length > 1 && (
        <>
          <button
            ...
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-40 bg-black/40 hover:bg-black/60 backdrop-blur-sm rounded-full p-1.5 sm:p-2 text-white opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          <button
            ...
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-40 bg-black/40 hover:bg-black/60 backdrop-blur-sm rounded-full p-1.5 sm:p-2 text-white opacity-0 group-hover:opacity-100 transition-opacity"
