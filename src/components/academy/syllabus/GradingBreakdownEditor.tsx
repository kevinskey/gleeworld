...
export const GradingBreakdownEditor: React.FC<Props> = ({
  ...
  const addItem = () => {
    onChange([...gradingBreakdown, { item: '', percentage: 0 }]);
  };

  const updateItem = (index: number, field: keyof GradingItem, value: string | number) => {
    onChange(gradingBreakdown.map((item, i) => 
  ...
  const updateScale = (grade: string, value: string) => {
    onScaleChange({ ...gradingScale, [grade]: value });
  };
        ...
        <CardContent className="space-y-4">
          {gradingBreakdown.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No grading items defined yet.</p>
              <p className="text-sm mt-1">Click "Add Item" to define grade components.</p>
            </div>
          ) : (
            <>
                ...
                <div key={index} className="grid grid-cols-12 gap-4 items-center">
                  <div className="col-span-8">
                    <Input
                      ...
                      placeholder="e.g., Participation, Midterm Exam, Final Project"
                    />
                  </div>
                  <div className="col-span-3">
                    <div className="relative">
                      <Input
                        ...
                        onChange={e => updateItem(index, 'percentage', parseInt(e.target.value) || 0)}
                        className="pr-8"
                      />
