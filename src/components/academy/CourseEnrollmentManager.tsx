...
export const CourseEnrollmentManager: React.FC<CourseEnrollmentManagerProps> = ({
  ...
  const loadEnrollments = async () => {
    try {
      ...
      } = await supabase.from('gw_course_enrollments').select('*').eq('course_id', courseId).order('enrolled_at', {
        ascending: false
      });
  ...
  const addEnrollment = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message?.includes('duplicate') ? "Student is already enrolled in this course" : "Failed to enroll student",
        variant: "destructive"
      });
    }
  };
  const updateEnrollmentStatus = async (enrollmentId: string, status: string) => {
    try {
      const {
        error
      } = await supabase.from('gw_course_enrollments').update({
  ...
  const removeEnrollment = async (enrollmentId: string) => {
    ...
    try {
      const {
        error
      } = await supabase.from('gw_course_enrollments').delete().eq('id', enrollmentId);
      if (error) throw error;
      toast({
        title: "Success",
  ...
  const filteredAndSortedEnrollments = enrollments.filter(enrollment => {
    const matchesSearch = enrollment.gw_profiles?.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) || enrollment.gw_profiles?.email?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'all' || enrollment.enrollment_status === statusFilter;
    const matchesGrade = gradeFilter === 'all' || gradeFilter === 'graded' && enrollment.grade || gradeFilter === 'ungraded' && !enrollment.grade;
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <ClasslistUploadDialog courses={[{
          id: courseId,
          title: courseTitle,
          course_code: courseCode
        }]} selectedCourseId={courseId} onUploadComplete={loadEnrollments} />
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={selectedUser} onValueChange={setSelectedUser}>
                    ...
                    <SelectContent>
                      {availableUsers.map(user => <SelectItem key={user.user_id} value={user.user_id}>
      ...
      <div className="grid gap-4">
        {filteredAndSortedEnrollments.length === 0 ? <Card>
          ...
          </Card> : filteredAndSortedEnrollments.map(enrollment => <Card key={enrollment.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate(`/${courseSlug}/instructor/student/${enrollment.user_id}`)}>
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div className="flex-1 min-w-[200px]">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      {enrollment.grade && <Badge variant="outline">
                          Grade: {enrollment.grade}
                        </Badge>}
                      {enrollment.academic_level && <Badge variant="secondary">
