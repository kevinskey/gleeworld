  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('youtube_channel_videos').select('*').eq('is_active', true).order('display_order', {
        ascending: true
      });
  ...
  return <div className="w-full">
      ...
      <div className={`bg-gradient-to-b from-[hsl(208,100%,14%)] to-[hsl(208,100%,10%)] py-5 ${isPaused ? 'overflow-x-auto' : 'overflow-hidden'} touch-pan-x`} onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} onTouchStart={() => setIsPaused(true)} onTouchEnd={() => setTimeout(() => setIsPaused(false), 3000)}>
        ...
        animationPlayState: isPaused ? 'paused' : 'running'
          ...
          </div> : <div className="text-white/60 text-sm py-4 px-5">No videos added yet</div>}
      </div>
      ...
      <Dialog open={!!selectedVideo} onOpenChange={handleCloseModal}>
        <DialogContent className="max-w-4xl w-[95vw] p-0 bg-black border-none overflow-hidden">
          ...
          <div className="relative">
            {/* Close button */}
            <button onClick={handleCloseModal} className="absolute -top-10 right-0 z-50 p-2 text-white/80 hover:text-white transition-colors">
              <X className="w-6 h-6" />
            </button>
            
            {/* Video embed */}
            {selectedVideo && <div className="aspect-video w-full">
                <iframe src={`https://www.youtube.com/embed/${selectedVideo.video_id}?autoplay=1&rel=0`} title={selectedVideo.title} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
              </div>}
            
            {/* Video title */}
            {selectedVideo && <div className="p-4 bg-gradient-to-t from-black to-transparent">
                <h3 className="text-white font-medium text-lg">{selectedVideo.title}</h3>
                {selectedVideo.description && <p className="text-white/70 text-sm mt-1 line-clamp-2">{selectedVideo.description}</p>}
              </div>}
          </div>
        </DialogContent>
      </Dialog>
