        ...
        <CollapsibleContent>
          <CardContent className="px-4 pb-4 py-[15px] bg-card">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-3">
              ...
              return <Button key={module.id} variant="outline" onClick={() => navigate(module.route)} className="h-[100px] py-3 px-2 flex flex-col items-center justify-start gap-2 !bg-white border-border hover:!bg-gray-50 hover:border-primary/30" style={{ background: 'white' }}>
                    <div className="flex-shrink-0 mt-2">
                      <IconComponent className="h-7 w-7 text-primary" />
                    </div>
                    <span className="text-[10px] text-center leading-tight line-clamp-2 !text-gray-800 px-0.5 font-medium break-words w-full sm:text-sm">
                      {module.title}
                    </span>
                  </Button>;
            })}
