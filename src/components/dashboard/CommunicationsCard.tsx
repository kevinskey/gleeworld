...
export const CommunicationsCard = ({
  ...
  return <Card className="bg-background/95 backdrop-blur-sm">
      <CardHeader className="py-3 px-3 sm:px-0">
        <CardTitle className="flex items-center gap-2 !text-white pl-[20px] px-[10px]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          return <div key={module.id} className={cn(isExpanded && module.isEmbedded && "col-span-2 md:col-span-4")}>
                <Collapsible open={isExpanded && module.isEmbedded} onOpenChange={() => handleModuleClick(module)}>
                  {/* Glossy Card Module */}
                  <div onClick={() => handleModuleClick(module)} className={cn("group cursor-pointer p-3 sm:p-4 flex flex-col items-center text-center transition-all duration-300", "bg-gradient-to-b from-primary/80 to-primary border border-primary/50 hover:border-primary", "shadow-lg hover:shadow-xl min-h-[120px] sm:min-h-[140px]", isExpanded && "border-primary")}>
                    {/* Icon Circle */}
                    <div className={cn("w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 flex items-center justify-center mb-2 sm:mb-3 transition-transform group-hover:scale-110", module.iconBg)}>
