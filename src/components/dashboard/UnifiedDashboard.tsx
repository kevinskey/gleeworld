  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.from('gw_profiles').select('user_id').eq('role', 'student').eq('status', 'active').limit(1).single();
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.from('gw_profiles').select('user_id').eq('role', 'member').eq('status', 'active').limit(1).single();
    ...
    return (
      <div className="min-h-screen">
        ...
        {showMemberNav && profile && (
          <div className="px-1 sm:px-6 pb-4 sm:pb-8">
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-4">
        ...
        {/* Section 3: My Modules (assigned modules based on exec role) */}
