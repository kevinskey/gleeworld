...
export const ResizableMessageCenter = ({
          ...
          <ScrollArea className="flex-1 px-2 py-1" ref={scrollRef}>
            ...
            return <div key={msg.id} className={`flex gap-1.5 ${isCurrentUser ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      addSuffix: true
                        ...
                        <Card className={`px-2 py-1.5 ${isCurrentUser ? 'bg-primary/15 border-primary' : 'bg-muted/50'}`}>
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent 
        side="right" 
        className="w-full max-w-full h-[100dvh] max-h-[100dvh] top-0 translate-y-0 rounded-none p-0 flex flex-col z-[9999] sm:w-[500px] sm:max-w-[500px] sm:max-h-[80vh] sm:top-1/2 sm:-translate-y-1/2 sm:rounded-lg"
      >
