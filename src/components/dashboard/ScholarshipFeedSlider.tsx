...
export const ScholarshipFeedSlider: React.FC = () => {
  const [isPaused, setIsPaused] = useState(false);

  const { data: items, isLoading } = useQuery({
    queryKey: ['scholarship-feed'],
    queryFn: async () => {
      const { data, error } = await supabase.functions.invoke('fetch-scholarship-feeds');
      if (error) throw error;
      if (!data?.success) throw new Error(data?.error || 'Failed to fetch scholarships');
      return data.items as ScholarshipItem[];
    },
  ...
  const animationDuration = items ? Math.max(items.length * 6, 20) : 20;
      ...
      >
        {isLoading ? (
          ...
          <div
            ...
            style={{
              animation: `scholarshipScrollInfinite ${animationDuration}s linear infinite`,
          ...
          >
            {duplicatedItems.map((item, index) => (
              ...
              >
                <div className="relative w-72 sm:w-80 lg:w-96 rounded-lg overflow-hidden border border-white/5 hover:border-amber-400/50 transition-all shadow-lg bg-[hsl(220,35%,12%)]">
                  {/* Image */}
                  {item.imageUrl ? (
                    ...
                    <div className="w-full h-36 bg-gradient-to-br from-amber-500/20 to-orange-600/20 flex items-center justify-center">
                      <GraduationCap className="h-12 w-12 text-amber-400/30" />
                    </div>
                  )}
          ...
          <div className="text-white/60 text-sm py-4 px-5">No scholarship news available</div>
        )}
      </div>

      {/* Infinite scroll keyframes */}
      <style>{`
        @keyframes scholarshipScrollInfinite {
...
export default ScholarshipFeedSlider;
