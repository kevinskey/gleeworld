...
export const MusicLibraryModule = () => {
  ...
  const uniqueComposers = useMemo(() => {
    const composers = musicLibrary.map(p => p.composer).filter((c): c is string => !!c && c.trim() !== '');
    return [...new Set(composers)].sort();
  }, [musicLibrary]);
  const uniqueVoicings = useMemo(() => {
    const voicings = musicLibrary.flatMap(p => p.voice_parts || []).filter((v): v is string => !!v && v.trim() !== '');
  ...
  const filteredMusic = useMemo(() => {
    let result = musicLibrary.filter(piece => {
      // Search filter
      const matchesSearch = !searchTerm || piece.title?.toLowerCase().includes(searchTerm.toLowerCase()) || piece.composer?.toLowerCase().includes(searchTerm.toLowerCase()) || piece.genre?.toLowerCase().includes(searchTerm.toLowerCase());

      // Composer filter
      const matchesComposer = composerFilter === 'all' || piece.composer === composerFilter;

      // Voicing filter
      const matchesVoicing = voicingFilter === 'all' || piece.voice_parts && piece.voice_parts.includes(voicingFilter);

      // Type filter
      const hasPdf = !!piece.pdf_url;
      const hasHardCopy = piece.has_hard_copy === true;
      let matchesType = true;
      if (typeFilter === 'pdf') matchesType = hasPdf && !hasHardCopy;else if (typeFilter === 'hard-copy') matchesType = hasHardCopy && !hasPdf;else if (typeFilter === 'both') matchesType = hasPdf && hasHardCopy;
  ...
  const activeFilterCount = [composerFilter !== 'all', voicingFilter !== 'all', typeFilter !== 'all', sortBy !== 'date-newest'].filter(Boolean).length;
  const clearFilters = () => {
  ...
  return <div className="h-full flex flex-col">
      ...
      <div className="py-2 md:py-4 border-b border-border bg-background">
        <div className="flex items-center gap-2 mb-3 md:mb-4 bg-[hsl(var(--brand-navy))] text-primary-foreground pl-[20px]">
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-4">
          <div className="relative flex-1 max-w-full md:max-w-md">
            ...
            <Input placeholder="Search..." className="pl-10 h-10 md:h-9 text-base md:text-sm" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
          ...
          <div className="flex gap-2 md:gap-3 flex-wrap">
            ...
            <Popover open={filterOpen} onOpenChange={setFilterOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="h-10 md:h-9 relative">
                  ...
                  <span>Filter</span>
                  {activeFilterCount > 0 && <Badge className="absolute -top-2 -right-2 h-5 w-5 p-0 flex items-center justify-center text-xs">
                      {activeFilterCount}
                    </Badge>}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-80" align="start">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {activeFilterCount > 0 && <Button variant="ghost" size="sm" onClick={clearFilters} className="h-8 text-xs">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={composerFilter} onValueChange={setComposerFilter}>
                      ...
                      <SelectContent>
                        ...
                        {uniqueComposers.map(composer => <SelectItem key={composer} value={composer}>{composer}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={voicingFilter} onValueChange={setVoicingFilter}>
                      ...
                      <SelectContent>
                        ...
                        {uniqueVoicings.map(voicing => <SelectItem key={voicing} value={voicing}>{voicing}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
              ...
              <Popover>
                ...
                <PopoverContent className="w-40" align="start">
                  <div className="space-y-1">
                    ...
                    {[1, 2, 3, 4, 5, 6].map(num => <Button key={num} variant={columnCount === num ? 'default' : 'ghost'} size="sm" className="w-full justify-start h-8" onClick={() => setColumnCount(num)}>
                        {num} columns
                      </Button>)}
                  </div>
                </PopoverContent>
            ...
            <div className="flex gap-1">
              <Button variant={viewMode === 'grid' ? 'default' : 'outline'} size="sm" onClick={() => setViewMode('grid')} className="h-10 md:h-9 px-3 md:px-2">
                <Grid className="w-4 h-4" />
              </Button>
              <Button variant={viewMode === 'list' ? 'default' : 'outline'} size="sm" onClick={() => setViewMode('list')} className="h-10 md:h-9 px-3 md:px-2">
      ...
      <Tabs defaultValue="all" className="flex-1 flex flex-col">
        ...
        <TabsContent value="all" className="flex-1 mt-0 flex flex-col min-h-0">
          <div className="flex-1 overflow-auto py-2 md:py-4 pb-0">
            {loading ? <div className="flex items-center justify-center py-12">
              ...
              </div> : filteredMusic.length === 0 ? <div className="flex items-center justify-center py-12">
                <p className="text-muted-foreground">
                  {searchTerm ? 'No music found matching your search' : 'No music in library yet'}
                </p>
              </div> : viewMode === 'grid' ? <div ref={gridRef} className="grid gap-2 md:gap-4 auto-rows-fr" style={{
            gridTemplateColumns: 'repeat(2, minmax(0, 1fr))'
          }}>
                {paginatedMusic.map(piece => <Card key={piece.id} className="p-2 md:p-4 hover:shadow-md transition-shadow cursor-pointer flex flex-col" onClick={() => {
              ...
              if (piece.xml_content && piece.xml_content.trim()) {
                ...
                toast({
                  ...
                  variant: "destructive"
                    ...
                    <div className="aspect-square bg-gradient-to-br from-primary/10 to-primary/5 rounded-lg mb-2 md:mb-3 flex items-center justify-center relative overflow-hidden">
                      {piece.pdf_url ? <PDFThumbnail pdfUrl={piece.pdf_url} alt={piece.title} title={piece.title} className="w-full h-full" /> : <Music className="w-8 h-8 md:w-12 md:h-12 text-primary/50" />}
                      <Button variant="ghost" size="icon" className={cn("absolute top-1 right-1 md:top-2 md:right-2 h-6 w-6 md:h-8 md:w-8 bg-background/80 backdrop-blur-sm hover:bg-background/90", isFavorite(piece.id) && "text-destructive")} onClick={e => {
                  e.stopPropagation();
                  toggleFavorite(piece.id);
                    ...
                    <div className="space-y-1 md:space-y-2 flex-1 flex flex-col">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {piece.voice_part && <Badge variant="outline" className="text-xs px-1 py-0">{piece.voice_part}</Badge>}
                        {piece.difficulty && <Badge className={`text-xs px-1 py-0 ${getDifficultyColor(piece.difficulty)}`}>
                            {piece.difficulty}
                          </Badge>}
                        {piece.genre && <Badge variant="secondary" className="text-xs px-1 py-0 hidden md:inline-flex">{piece.genre}</Badge>}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-1 md:pt-2 mt-auto">
                        ...
                        <div className="flex gap-1 ml-auto">
                          {piece.audio_url && <Button size="sm" variant="outline" className="h-7 w-7 md:h-8 md:w-8 p-0" onClick={e => {
                      e.stopPropagation();
                      togglePlay(piece.id);
                    }}>
                              {isPlaying === piece.id ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3" />}
                            </Button>}
                          {piece.pdf_url && <Button size="sm" variant="outline" className="h-7 w-7 md:h-8 md:w-8 p-0" onClick={e => {
                      e.stopPropagation();
                ...
                {paginatedMusic.map(piece => <Card key={piece.id} className="p-4 hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => {
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          {piece.voice_part && <Badge variant="outline" className="text-xs">{piece.voice_part}</Badge>}
                          {piece.difficulty && <Badge className={`text-xs ${getDifficultyColor(piece.difficulty)}`}>
                              {piece.difficulty}
                            </Badge>}
                          {piece.genre && <Badge variant="secondary" className="text-xs">{piece.genre}</Badge>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" className={cn("h-8 w-8", isFavorite(piece.id) && "text-destructive")} onClick={e => {
                      e.stopPropagation();
                      toggleFavorite(piece.id);
                    }}>
                            <Heart className={cn("h-4 w-4", isFavorite(piece.id) && "fill-current")} />
                          </Button>
                          {piece.audio_url && <Button size="sm" variant="outline" onClick={e => {
                      e.stopPropagation();
                      togglePlay(piece.id);
                    }}>
                              {isPlaying === piece.id ? <Pause className="w-4 h-4 mr-2" /> : <Play className="w-4 h-4 mr-2" />}
                              {isPlaying === piece.id ? 'Pause' : 'Play'}
                            </Button>}
                          {piece.pdf_url && <Button size="sm" variant="outline" onClick={e => {
                      e.stopPropagation();
                      window.open(piece.pdf_url, '_blank');
                    }}>
          ...
          {totalPages > 1 && <div className="flex items-center justify-center gap-2 py-4 border-t border-border bg-background">
              <Button variant="outline" size="sm" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="h-9 md:h-8 px-3 md:px-4">
                Previous
              </Button>
              <span className="text-xs md:text-sm text-muted-foreground px-2">
                Page {currentPage} of {totalPages}
              </span>
              <Button variant="outline" size="sm" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="h-9 md:h-8 px-3 md:px-4">
                Next
              </Button>
            </div>}
        </TabsContent>
        ...
        <TabsContent value="recent" className="flex-1">
          <ScrollArea className="flex-1 py-4">
            {loading ? <div className="flex items-center justify-center py-12">
              ...
              </div> : <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                {musicLibrary.slice(0, 12).map(piece => <Card key={piece.id} className="p-4 cursor-pointer hover:shadow-md transition-shadow" onClick={() => piece.pdf_url && openPdfViewer(piece)}>
                    <h3 className="font-medium text-sm mb-1">{piece.title}</h3>
        ...
        <TabsContent value="favorites" className="flex-1">
          <ScrollArea className="flex-1 py-2 md:py-4">
            {favoritedMusic.length === 0 ? <div className="flex items-center justify-center text-muted-foreground py-12">
                <p>No favorite pieces yet. Click the heart icon on any piece to add it to your favorites!</p>
              </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {favoritedMusic.map(piece => <Card key={piece.id} className="p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => piece.pdf_url && openPdfViewer(piece)}>
                    <div className="aspect-square bg-gradient-to-br from-primary/10 to-primary/5 rounded-lg mb-3 flex items-center justify-center relative overflow-hidden">
                      {piece.pdf_url ? <PDFThumbnail pdfUrl={piece.pdf_url} alt={piece.title} title={piece.title} className="w-full h-full" /> : <Music className="w-12 h-12 text-primary/50" />}
                      <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-8 w-8 text-destructive bg-background/80 backdrop-blur-sm hover:bg-background/90" onClick={e => {
                  e.stopPropagation();
                  toggleFavorite(piece.id);
                }}>
                    ...
                    <div className="flex gap-1">
                      {piece.pdf_url && <Button size="sm" variant="outline" className="flex-1" onClick={e => {
                  e.stopPropagation();
                  window.open(piece.pdf_url, '_blank');
                }}>
      ...
      {selectedPdf && <InAppPDFViewerDialog open={!!selectedPdf} onOpenChange={open => {
      console.log('MusicLibraryModule onOpenChange:', open);
      if (!open) {
        closeGuardUntilRef.current = Date.now() + 400;
        setSelectedPdf(null);
      }
    }} pdfUrl={selectedPdf.url} title={selectedPdf.title} musicId={selectedPdf.id} />}

      {selectedMusicXML && <MusicXMLViewer musicxml={selectedMusicXML.content} onClose={() => setSelectedMusicXML(null)} title={selectedMusicXML.title} />}
    </div>;
};