      ...
      <div className="w-full py-4 bg-muted/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-3 overflow-x-auto pb-3">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex-shrink-0 w-32 h-44 bg-muted animate-pulse rounded-lg" />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="w-full py-4 bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold">Shop Glee Merch</h2>
          <button 
        ...
        <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide">
          {products.map((product) => (
            <div
              key={product.id}
              onClick={() => navigate('/shop')}
              className="flex-shrink-0 w-32 bg-card rounded-lg border shadow-sm overflow-hidden cursor-pointer hover:shadow-md transition-shadow"
            >
