...
const mockCourseEvents: CourseEvent[] = [{
  ...
  title: 'Baton Technique Workshop',
  type: 'class',
  time: '2:00 PM'
}, {
  id: '2',
  date: new Date(2025, 10, 10),
  title: 'Score Analysis Assignment Due',
  type: 'assignment',
  time: '11:59 PM'
}, {
  id: '3',
  ...
  title: 'Conducting Project Presentation',
  type: 'test',
  time: '2:00 PM'
}, {
  id: '6',
              ...
              return <button key={idx} onClick={() => setSelectedDate(day)} className={`
                    ...
                    <div className="flex flex-col items-center justify-center h-full">
                      ...
                      {dayEvents.length > 0 && <div className="flex gap-0.5 mt-0.5">
                          {dayEvents.slice(0, 3).map((event, i) => <div key={i} className={`w-1 h-1 rounded-full ${event.type === 'exam' ? 'bg-red-500' : event.type === 'test' ? 'bg-orange-500' : event.type === 'assignment' ? 'bg-green-500' : event.type === 'class' ? 'bg-blue-500' : event.type === 'office-hours' ? 'bg-purple-500' : 'bg-yellow-500'}`} />)}
                        </div>}
                    </div>
                  </button>;
            })}
        ...
        <Card>
          ...
          <CardContent>
            {selectedDateEvents.length > 0 ? <div className="space-y-3">
                {selectedDateEvents.map(event => <div key={event.id} className={`border rounded-lg p-3 ${getEventColor(event.type)}`}>
                    <div className="flex items-start gap-2">
                      {getEventIcon(event.type)}
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-sm">{event.title}</h4>
                        {event.time && <p className="text-xs opacity-80 mt-0.5">
              ...
              </p> : <p className="text-sm text-muted-foreground text-center py-8">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {mockCourseEvents.filter(event => event.date >= new Date()).sort((a, b) => a.date.getTime() - b.date.getTime()).slice(0, 5).map(event => <div key={event.id} className={`flex items-center justify-between p-3 border rounded-lg ${getEventColor(event.type)}`}>
