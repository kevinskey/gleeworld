  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          emailRedirectTo: `${window.location.origin}/graduates`,
          data: {
      ...
      console.error('Graduate signup error:', error);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-purple-50 p-4 rounded-lg">
            ...
            <ul className="text-sm text-purple-800 space-y-1">
              <li>• Access to graduates-only events and content</li>
          ...
          >
            {isSubmitting ? "Creating Account..." : "Join as Graduate"}
          </Button>
        </form>
      </CardContent>
    </Card>
