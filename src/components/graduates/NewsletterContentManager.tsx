...
export const NewsletterContentManager = ({ newsletterId }: NewsletterContentManagerProps) => {
  ...
  // Spotlights
  const [graduatesSpotlight, setGraduatesSpotlight] = useState({
  ...
  const handleSaveSpotlights = async () => {
    try {
      ...
      if (graduatesSpotlight.name) {
        spotlightsToInsert.push({
          ...graduatesSpotlight,
          newsletter_id: newsletterId,
          spotlight_type: 'graduates',
          display_order: 0
        });
      }
        ...
        {activeTab === 'spotlights' && (
          <div className="space-y-6">
            <div className="p-4 border rounded-lg space-y-3">
              <Label className="text-base font-semibold">Graduates Spotlight</Label>
              <Input
                placeholder="Name"
                value={graduatesSpotlight.name}
                onChange={(e) => setGraduatesSpotlight({ ...graduatesSpotlight, name: e.target.value })}
              />
              <Input
                placeholder="Title (e.g., Class of 2015)"
                value={graduatesSpotlight.title}
                onChange={(e) => setGraduatesSpotlight({ ...graduatesSpotlight, title: e.target.value })}
              />
              <Input
                placeholder="Photo URL"
                value={graduatesSpotlight.photo_url}
                onChange={(e) => setGraduatesSpotlight({ ...graduatesSpotlight, photo_url: e.target.value })}
              />
              <Textarea
                placeholder="Description"
                value={graduatesSpotlight.description}
                onChange={(e) => setGraduatesSpotlight({ ...graduatesSpotlight, description: e.target.value })}
              />
            </div>
