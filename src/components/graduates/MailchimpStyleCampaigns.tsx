...
export const MailchimpStyleCampaigns = () => {
  ...
  const [graduates, setGraduates] = useState<GraduatesUser[]>([]);
  const [filteredGraduates, setFilteredGraduates] = useState<GraduatesUser[]>([]);
  ...
  useEffect(() => {
    applySegmentation();
  }, [graduates, segmentFilter, searchQuery]);
  ...
  const loadGraduates = async () => {
    ...
    try {
      ...
      const graduatesIds = rolesData.map(r => r.user_id);

      if (graduatesIds.length === 0) {
        setGraduates([]);
        setLoading(false);
        return;
      }
      ...
      setGraduates(profilesData || []);
    } catch (error: any) {
      toast.error("Failed to load graduates", { description: error.message });
    } finally {
      setLoading(false);
    }
  };
  ...
  const selectAll = () => {
    if (filteredGraduates.every(a => selectedRecipients.has(a.user_id))) {
      const newSet = new Set(selectedRecipients);
      filteredGraduates.forEach(a => newSet.delete(a.user_id));
      setSelectedRecipients(newSet);
    } else {
      setSelectedRecipients(new Set([...selectedRecipients, ...filteredGraduates.map(a => a.user_id)]));
    }
  };
  ...
  const sendCampaign = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      if (!isDraft) {
        // Send the email
        const { data, error } = await supabase.functions.invoke('send-graduates-email', {
        ...
        toast.success("Campaign sent successfully!", {
          description: `Email sent to ${recipients.length} graduates`
        });
      } else {
        // Save as draft
        const { error: saveError } = await supabase
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Create, manage, and track email campaigns to graduates
          </p>
        </div>
        <Badge variant="outline" className="text-lg px-4 py-2">
          {graduates.length} Total Graduates
        </Badge>
      </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <Button variant="outline" size="sm" onClick={selectAll}>
                  {filteredGraduates.every(a => selectedRecipients.has(a.user_id)) && filteredGraduates.length > 0
              ...
              <ScrollArea className="h-[300px] border rounded-lg p-4">
                <div className="space-y-2">
                  {filteredGraduates.map((graduate) => (
                    <div key={graduate.user_id} className="flex items-center space-x-3 p-2 hover:bg-muted rounded">
                      <Checkbox
                        checked={selectedRecipients.has(graduate.user_id)}
                        onCheckedChange={() => toggleRecipient(graduate.user_id)}
                      />
                      <div className="flex-1">
                        <p className="text-sm font-medium">{graduate.full_name || graduate.email}</p>
                        <p className="text-xs text-muted-foreground">{graduate.email}</p>
                      </div>
                      {graduate.graduation_year && (
                        <Badge variant="outline" className="text-xs">
                          '{graduate.graduation_year.toString().slice(-2)}
                        </Badge>
                      )}
                    </div>
                  ))}
                  ...
                  {filteredGraduates.length === 0 && (
                    <p className="text-sm text-muted-foreground text-center py-8">
                      {searchQuery ? `No graduates found matching "${searchQuery}"` : 'No graduates available'}
                    </p>
                  )}
                </div>
              </ScrollArea>
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{graduates.length}</div>
                <p className="text-xs text-muted-foreground">Active graduates</p>
              </CardContent>
            </Card>
          </div>
