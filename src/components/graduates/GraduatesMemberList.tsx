...
export const GraduatesMemberList = () => {
  const [members, setMembers] = useState<GraduatesMember[]>([]);
  const [filteredMembers, setFilteredMembers] = useState<GraduatesMember[]>([]);
  ...
  const fetchMembers = async () => {
    try {
      ...
      console.error('Error fetching graduates members:', error);
      toast.error('Failed to load graduates members');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('send-graduates-email', {
        body: {
          recipients,
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Graduates Members ({filteredMembers.length})
            </CardTitle>
          ...
          <Dialog open={emailDialogOpen} onOpenChange={setEmailDialogOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>Send Email to Selected Graduates</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label>Recipients</Label>
                  <div className="text-sm text-muted-foreground">
                    {selectedMembers.size} graduates member(s) selected
                  </div>
                </div>
                <div className="space-y-2">
