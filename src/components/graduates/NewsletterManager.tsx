...
export const NewsletterManager = () => {
  ...
  useEffect(() => {
    const autoTitle = `SCGC Graduates Newsletter ${year} Vol. ${volume}`;
    setTitle(autoTitle);
  }, [year, volume]);
  ...
  const autoSave = useCallback(async () => {
    ...
    try {
      ...
      if (coverImageFile) {
        ...
        const uploadedUrl = await uploadFile(coverImageFile, 'graduates-docs', coverPath);
        if (uploadedUrl) {
      ...
      if (pdfFile) {
        ...
        const uploadedUrl = await uploadFile(pdfFile, 'graduates-docs', pdfPath);
        if (uploadedUrl) {
  ...
  const handleSendBroadcast = async () => {
    ...
    try {
      ...
      toast.success(
        "Newsletter broadcast sent successfully to all verified graduates!",
        { duration: 5000 }
      );
    } catch (error: any) {
        ...
        <div className="flex gap-2">
          {currentNewsletterId && isPublished && (
            ...
            >
              ...
              {sendingEmails ? "Sending..." : "Send to All Graduates"}
            </Button>
          )}
