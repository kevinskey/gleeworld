  ...
  useEffect(() => {
    const fetchFormatting = async () => {
      try {
        ...
        console.error('Failed to load graduates hero title formatting:', err);
      }
    };
  ...
  useEffect(() => {
    if (titleFormatting?.fontFamily) {
      ...
      if (primary && primary !== 'inherit') {
        ...
        WebFont.load({
          google: { families },
          active: () => console.log('✅ GraduatesHero font loaded:', families.join(',')),
          inactive: () => console.log('❌ GraduatesHero font failed:', families.join(',')),
        });
      }
    }
  }, [titleFormatting]);
