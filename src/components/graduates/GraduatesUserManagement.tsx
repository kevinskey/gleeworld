...
import { GraduatesBulkImport } from './GraduatesBulkImport';
import { GraduatesProfileEditor } from './GraduatesProfileEditor';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
export const GraduatesUserManagement = () => {
  ...
  const fetchGraduatesUsers = async () => {
    setLoading(true);
    try {
      // Fetch ALL graduates profiles (removed limit)
      const {
        data: profileData,
        error: profileError,
        count
      ...
      if (profileError) {
        ...
        toast.error('Failed to load graduates: ' + profileError.message);
        setUsers([]);
        setTotalCount(0);
      } else {
      ...
      {paginatedUsers.map(user => <Card key={user.id} className="overflow-hidden hover:shadow-lg transition-shadow">
          ...
          <CardContent className="pt-4 space-y-3">
            ...
            <div className="space-y-2">
              ...
              {(user.city || user.state) && <div className="flex items-center gap-2 text-sm">
            ...
            <div className="space-y-2 pt-2 border-t">
              {user.graduation_year && <div className="flex items-center gap-2 text-sm">
                  <GraduationCap className="h-4 w-4 text-muted-foreground" />
                  <span>Class of {user.graduation_year}</span>
                </div>}
              {user.major && <div className="flex items-center gap-2 text-sm">
                  <Award className="h-4 w-4 text-muted-foreground" />
                  <span>{user.major}</span>
                </div>}
              {user.current_employer && <div className="flex items-center gap-2 text-sm">
                  <Briefcase className="h-4 w-4 text-muted-foreground" />
                  <span className="truncate">{user.current_employer}</span>
                </div>}
              {user.current_position && <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <span className="ml-6 truncate">{user.current_position}</span>
                </div>}
            </div>

            {/* Glee Club Info */}
            {(user.voice_part || user.section_leader) && <div className="space-y-2 pt-2 border-t">
                {user.voice_part && <div className="flex items-center gap-2 text-sm">
            ...
            {/* Bio */}
            {user.bio && <div className="pt-2 border-t">
                <p className="text-sm text-muted-foreground line-clamp-3">{user.bio}</p>
              </div>}

            {/* Social Links */}
            {(user.linkedin_url || user.instagram_handle || user.website_url) && <div className="flex gap-2 pt-2 border-t">
                {user.linkedin_url && <Button variant="outline" size="sm" asChild>
                    <a href={user.linkedin_url} target="_blank" rel="noopener noreferrer">
                      <Globe className="h-3 w-3 mr-1" />
                      LinkedIn
                    </a>
                  </Button>}
                {user.website_url && <Button variant="outline" size="sm" asChild>
          ...
          {paginatedUsers.map(user => <TableRow key={user.id}>
              ...
              <TableCell>
                <div className="space-y-1">
                  ...
                  {(user.city || user.state) && <div className="text-sm text-muted-foreground">
  ...
  return <div className="space-y-6">
        ...
        <CardContent>
          {loading ? <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Loading graduates...</p>
              </div>
            </div> : filteredUsers.length === 0 ? <div className="flex flex-col items-center justify-center py-12 text-center">
              <UserCheck className="h-12 w-12 text-muted-foreground mb-4" />
              <h3 className="text-lg font-semibold mb-2">No Graduates Found</h3>
              <p className="text-muted-foreground mb-4">
                {search ? 'No graduates match your search criteria.' : 'No users have been assigned the graduate role yet.'}
              </p>
              {!search && <p className="text-sm text-muted-foreground">
      ...
      {editingUser && <GraduatesProfileEditor user={editingUser} open={!!editingUser} onOpenChange={open => !open && setEditingUser(null)} onSuccess={fetchGraduatesUsers} />}
    </div>;
};