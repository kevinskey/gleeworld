          ...
          <Card className="border-border">
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon-sm" aria-label="Previous month" onClick={() => setCalendarMonth(prev => subMonths(prev, 1))}>
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <span className="text-sm font-medium min-w-[120px] text-center" style={{ color: 'hsl(222, 47%, 11%)' }}>
                  {format(calendarMonth, 'MMMM yyyy')}
                </span>
                <Button variant="ghost" size="icon-sm" aria-label="Next month" onClick={() => setCalendarMonth(prev => addDays(startOfMonth(prev), 32))}>
