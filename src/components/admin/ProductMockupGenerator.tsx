...
export const ProductMockupGenerator = () => {
  ...
  const loadProducts = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('gw_products').select('id, title, product_type, images').order('product_type', {
        ascending: true
      });
  ...
  const generateMockup = async (product: Product) => {
    try {
      ...
      setMockupStatuses(prev => prev.map(status => status.productId === product.id ? {
        ...status,
        status: 'generating'
      } : status));
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-product-mockup', {
        body: {
          productId: product.id,
          productName: product.title,
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        return <Card key={status.productId} className="overflow-hidden">
              <CardContent className="p-4">
                ...
                {status.imageUrl && <div className="aspect-square rounded-md overflow-hidden bg-muted mb-3">
                    <img src={status.imageUrl} alt={`${status.productName} mockup`} className="w-full h-full object-cover" />
                  </div>}

                {status.status === 'error' && status.error && <p className="text-xs text-red-500 mb-3">{status.error}</p>}

                {(status.status === 'pending' || status.status === 'error') && <Button size="sm" variant="outline" onClick={() => generateMockup(product)} disabled={false} className="w-full">
