...
interface GraduatesDashboardData {
  reunionEvents: GraduatesEvent[];
  graduatesProfiles: GraduatesProfile[];
...
export const GraduatesDashboardMonitor = () => {
  const [dashboardData, setDashboardData] = useState<GraduatesDashboardData>({
    reunionEvents: [],
    graduatesProfiles: [],
    memoryWall: [],
    mentorships: [],
  ...
  const [analytics, setAnalytics] = useState<GraduatesAnalytics>({
    totalGraduates: 0,
    activeMentors: 0,
    upcomingReunions: 0,
    recentMemories: 0
  ...
  const fetchGraduatesDashboardData = async () => {
    try {
      setLoading(true);
      
      // Fetch graduates profiles
      const { data: graduatesProfiles, count: graduatesCount } = await supabase
        .from('gw_profiles')
        .select('*', { count: 'exact' })
        .eq('role', 'graduate')
      ...
      // Fetch reunion/graduates events
      ...
      // Fetch announcements for graduates
      const { data: announcements } = await supabase
        .from('gw_announcements')
        .select('*')
        .or('target_audience.eq.graduates,target_audience.eq.all')
        .order('created_at', { ascending: false })
        .limit(5);

      setDashboardData({
        reunionEvents: events || [],
        graduatesProfiles: graduatesProfiles || [],
        memoryWall: [],
        mentorships: [],
      ...
      setAnalytics({
        totalGraduates: graduatesCount || 0,
      ...
      console.error('Error fetching graduates dashboard data:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Monitor and manage what graduates see on their dashboard
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button 
            onClick={fetchGraduatesDashboardData} 
            disabled={loading}
            variant="outline"
            size="sm"
          >
          ...
          <Button asChild>
            <a href="/graduates" target="_blank" rel="noopener noreferrer">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{analytics.totalGraduates}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Graduates Directory & Profiles
              </CardTitle>
            </CardHeader>
            <CardContent>
              {dashboardData.graduatesProfiles.length > 0 ? (
                <div className="space-y-3">
                  {dashboardData.graduatesProfiles.slice(0, 10).map((graduate) => (
                    <div key={graduate.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <h4 className="font-medium">{graduate.full_name}</h4>
                          {getClassYearBadge(graduate.graduation_year || graduate.class_year)}
                          {graduate.mentor_opt_in && (
                            <Badge variant="secondary">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {graduate.workplace && `${graduate.workplace} • `}
                          {graduate.current_city || 'Location not specified'}
                        </p>
                      </div>
                      <Button variant="outline" size="sm">
                  ...
                  <div className="text-center pt-4">
                    <Button variant="outline">
                      View All {analytics.totalGraduates} Graduates
                    </Button>
                  </div>
                </div>
              ) : (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>No graduates profiles found</p>
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Graduates offering mentorship to current students
                    </p>
                  </div>
            ...
            <CardContent>
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm mb-4">
                  A place for graduates to share photos, stories, and memories from their time in Glee Club
                </p>
                <Button variant="outline">
                  <Settings className="h-4 w-4 mr-2" />
                  Set Up Memory Wall
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Graduates Updates & Announcements
              </CardTitle>
            </CardHeader>
            <CardContent>
              {dashboardData.announcements.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>No recent announcements for graduates</p>
                  <Button className="mt-4" variant="outline">
                    <Settings className="h-4 w-4 mr-2" />
                    Create Graduates Announcement
                  </Button>
                </div>
              )}
            </CardContent>
