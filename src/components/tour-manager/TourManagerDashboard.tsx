...
export const TourManagerDashboard = ({
  ...
  return <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex flex-col lg:flex-row">
      {/* Sidebar - hidden on mobile, shown on desktop */}
      <aside className={cn("fixed inset-y-0 left-0 z-40 w-56 bg-[#003666] border-r border-[#002244] transform transition-transform duration-200 ease-in-out lg:relative lg:translate-x-0 flex-shrink-0", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex flex-col h-full">
          {/* Sidebar Header */}
          <div className="px-3 py-3 border-b border-[#004488] flex items-center justify-between bg-[#002244]">
            ...
            <Button variant="ghost" size="icon" className="lg:hidden h-8 w-8 text-white hover:bg-[#004488]" onClick={() => setSidebarOpen(false)}>
              <X className="h-4 w-4" />
            </Button>
          </div>

          {/* Navigation */}
          <ScrollArea className="flex-1 py-2 bg-[#003666]">
          ...
          {/* Sidebar Footer */}
          <div className="p-3 border-t border-[#004488] bg-[#002244]">
      ...
      <main className="flex-1 flex flex-col min-w-0 pb-20 lg:pb-0">
        {/* Top Bar - Blue stripe header */}
        <header className="sticky top-0 z-20 bg-[#003666] border-b border-[#002244]">
          <div className="flex items-center justify-between px-3 lg:px-4 h-12">
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="icon" className="lg:hidden h-8 w-8 flex-shrink-0 text-white hover:bg-white/10" onClick={() => setSidebarOpen(!sidebarOpen)}>
              ...
              <span className="text-xs text-blue-200 hidden sm:inline truncate">{currentContent.description}</span>
            </div>
            <div className="relative w-48 hidden md:block">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-blue-200" />
              <Input placeholder="Search" className="pl-8 h-8 text-sm bg-white/10 border-white/20 text-white placeholder:text-blue-200 focus-visible:ring-1 focus-visible:ring-white/30" />
            </div>
          </div>
        </header>

        {/* Content Area - Light background for contrast */}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 lg:hidden z-30 safe-area-inset-bottom">
