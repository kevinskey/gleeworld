          ...
          <ScrollArea className="h-[300px] pr-4">
            <div className="space-y-1">
              {milestones.map(milestone => (
                <div key={milestone.id} className={cn("flex items-center gap-2 p-2 rounded-md group hover:bg-muted/50 transition-colors", milestone.completed && "opacity-60")}>
                  ...
                  {editingId === milestone.id ? (
                    ...
                    <>
                      {/* Sign-off dropdown */}
                      <Popover open={openSignOffId === milestone.id} onOpenChange={(open) => setOpenSignOffId(open ? milestone.id : null)}>
                        <PopoverTrigger asChild>
                          <button className={cn(
                            "flex-1 text-left text-xs text-white hover:underline cursor-pointer",
