...
export function GraduatesPortalModule({ user, isFullPage, onNavigate }: ModuleProps) {
  ...
  const [contents, setContents] = useState<GraduatesContent[]>([]);
  const [graduatesUsers, setGraduatesUsers] = useState<GraduatesProfile[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedContent, setSelectedContent] = useState<GraduatesContent | null>(null);
  ...
  const fetchContent = async () => {
    try {
      ...
      console.error('Error fetching graduates content:', error);
  ...
  const fetchGraduatesUsers = async () => {
    try {
      ...
      if (error) throw error;
      setGraduatesUsers(data || []);
    } catch (error) {
      console.error('Error fetching graduates users:', error);
      toast.error('Failed to load graduates users');
    }
  };
  ...
  const openEditDialog = (content?: GraduatesContent) => {
    if (content) {
  ...
  if (roleLoading || loading) {
    return (
      <ModuleWrapper title="Graduates Portal Management" icon={Layout}>
      ...
      <ModuleWrapper title="Graduates Portal Management" icon={Layout}>
        <Card>
          <CardContent className="text-center py-12">
            ...
            <Button onClick={() => onNavigate?.('graduates-portal')}>
              View Graduates Portal
            </Button>
          </CardContent>
        </Card>
      </ModuleWrapper>
  ...
  const groupedContents = contents.reduce((acc, content) => {
    ...
    return acc;
  }, {} as Record<string, GraduatesContent[]>);
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage graduates portal and landing page content
            </p>
          </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Graduates Users
                  <Badge variant="secondary">{graduatesUsers.length}</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {graduatesUsers.length > 0 ? (
                  <div className="space-y-3">
                    {graduatesUsers.map((graduate) => (
                      <div key={graduate.user_id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center gap-4">
                          {graduate.headshot_url ? (
                            <img 
                              src={graduate.headshot_url} 
                          ...
                          <div>
                            <h4 className="font-semibold">{graduate.full_name}</h4>
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant={graduate.verified ? "default" : "secondary"}>
                            {graduate.verified ? "Verified" : "Unverified"}
                          </Badge>
                          <div className="text-xs text-muted-foreground text-right">
                            <div className="flex items-center gap-1">
                              <Calendar className="h-3 w-3" />
                              Joined {format(new Date(graduate.created_at), 'MMM yyyy')}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold mb-2">No Graduates Users</h3>
                    <p className="text-muted-foreground">
                      No users with the 'graduate' role found.
                    </p>
                  </div>
                )}
              </CardContent>
