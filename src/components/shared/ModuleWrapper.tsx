...
export const ModuleWrapper = ({
  ...
  const content = <div className={`${containerClass} ${className}`}>
      ...
      top: 'var(--app-header-offset)'
        ...
        <div className="flex items-center justify-between card-spacing">
          <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
            {showBack && <Button variant="outline" size="sm" onClick={onBack} aria-label="Go back" className="touch-target bg-background text-foreground border-border">
          ...
          <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
            {headerActions}
            {collapsible && <Button variant="ghost" size="sm" aria-controls={`${id}-content`} aria-expanded={!collapsed} onClick={() => setCollapsed(v => !v)} title={collapsed ? 'Expand module' : 'Collapse module'} className="touch-target">
      ...
      {/* Module Content */}
      {!collapsed && <div id={`${id}-content`} className="section-spacing pt-2 md:pt-4">
          {isLoading ? <Card>
...
export const ModuleCard = ({
  ...
  return <Card className={`hover:shadow-md transition-all ${onClick ? 'cursor-pointer hover:border-primary/20' : ''} ${className}`} onClick={onClick}>
      ...
      {children && <CardContent className="card-compact pt-0">
          {children}
        </CardContent>}
    </Card>;
};
  ...
  return <Card>
      <CardContent className="card-compact">
        <div className="flex items-center justify-between">
          ...
          {Icon && <Icon className={`h-5 w-5 md:h-6 md:w-6 text-${iconColor}-600 flex-shrink-0`} />}
        </div>
        {trend && <div className="mt-1 md:mt-2 flex items-center gap-1">
