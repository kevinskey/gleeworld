...
export const ModulePageHeader: React.FC<ModulePageHeaderProps> = ({
  ...
  return (
    <div 
      className={`relative z-10 rounded-lg border-2 shadow-lg h-12 sm:h-14 px-3 sm:px-6 flex items-center ${
        isHbcuTheme 
    ...
    >
      <div className="flex items-center justify-between gap-2 w-full">
        ...
        <div className="flex items-center gap-2 flex-1 justify-center">
          {Icon && (
            <Icon 
              className={`h-4 w-4 sm:h-5 sm:w-5 shrink-0 ${isHbcuTheme ? 'text-primary' : 'text-destructive'}`}
            />
          )}
          <h1 
            className={`text-sm sm:text-xl lg:text-2xl font-bold tracking-wide font-mono uppercase ${
