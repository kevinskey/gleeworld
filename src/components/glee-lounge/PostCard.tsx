...
export function PostCard({
  ...
  const handleDelete = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('gw_social_posts').delete().eq('id', post.id);
  ...
  return <>
      <Card className="mb-4">
        <CardContent className="pt-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {isOwnPost ? <DropdownMenuItem onClick={handleDelete} disabled={isDeleting} className="text-destructive focus:text-destructive">
                    ...
                    Delete post
                  </DropdownMenuItem> : <DropdownMenuItem onClick={() => setShowReportDialog(true)}>
          ...
          {/* Comments */}
          <PostComments postId={post.id} commentCount={post.comment_count} userProfile={userProfile} />
        </CardContent>
      </Card>
      ...
      <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}>
        <DialogContent className="max-w-4xl p-0 bg-black/95 border-none">
          <div className="relative flex items-center justify-center min-h-[50vh]">
            ...
            {post.media_urls && post.media_urls.length > 1 && <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white text-sm">
