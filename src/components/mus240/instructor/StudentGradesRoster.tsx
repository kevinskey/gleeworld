...
export const StudentGradesRoster: React.FC = () => {
  ...
  const fetchAssignmentPoints = async (studentId: string) => {
    ...
    const {
      data: journals
    } = await supabase.from('mus240_journal_grades').select('overall_score').eq('student_id', studentId);
    const journalPoints = (journals || []).reduce((sum, j) => sum + (j.overall_score || 0), 0);

    // Fetch all assignment submissions (excluding journals)
    const {
      data: submissions
    } = await supabase.from('assignment_submissions').select('grade').eq('student_id', studentId);
    ...
    return {
      earned: journalPoints + assignmentPoints,
      possible: 650
    };
  };
  const fetchParticipationPoints = async (studentId: string) => {
    ...
    const {
      data: participation
    } = await supabase.from('mus240_participation_grades').select('points_earned').eq('student_id', studentId).single();

    // Fetch poll responses
    const {
      data: pollResponses
    } = await supabase.from('mus240_poll_responses').select('poll_id').eq('student_id', studentId);
    const {
      data: allPolls
    } = await supabase.from('mus240_polls').select('id');
  ...
  const fetchSubmissionsCount = async (studentId: string) => {
    const {
      data: submissions
    } = await supabase.from('assignment_submissions').select('id').eq('student_id', studentId);
  ...
  const exportGrades = () => {
    const csv = [['Student Name', 'Email', 'Assignment Points', 'Participation', 'Overall Score', 'Overall %', 'Letter Grade', 'Submissions'].join(','), ...students.map(s => [s.student_name, s.student_email, `${s.assignment_points}/650`, `${s.participation_points.toFixed(0)}/75`, `${s.overall_score.toFixed(0)}/725`, (s.overall_score / s.overall_possible * 100).toFixed(1), s.letter_grade, s.submissions_count].join(','))].join('\n');
  ...
  const filteredStudents = students.filter(s => s.student_name.toLowerCase().includes(searchTerm.toLowerCase()) || s.student_email.toLowerCase().includes(searchTerm.toLowerCase()));
  if (loading) {
  ...
  return <Card>
      ...
      <CardContent className="space-y-4">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right text-primary-foreground">Assignment Points</TableHead>
                <TableHead className="text-right text-primary-foreground">Participation</TableHead>
                <TableHead className="text-right text-primary-foreground">Overall Score</TableHead>
                <TableHead className="text-center text-primary-foreground">Grade</TableHead>
                <TableHead className="text-right text-primary-foreground">Submissions</TableHead>
            ...
            <TableBody>
              ...
              return <TableRow key={student.student_id} className="hover:bg-muted/50">
                    <TableCell>
                      <div>
                    ...
                    <TableCell className="text-right">
                      <div className="space-y-1">
                        ...
                        width: `${Math.min(student.assignment_points / 650 * 100, 100)}%`
                      }} />
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="space-y-1">
                        ...
                        <div className="w-24 bg-muted rounded-full h-1.5 ml-auto">
                          <div className="bg-primary h-1.5 rounded-full" style={{
                        width: `${Math.min(student.participation_points / 75 * 100, 100)}%`
                      }} />
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-center">
                      <Button variant="ghost" size="sm" onClick={() => navigate(`/mus-240/instructor/student/${student.student_id}`)}>
                        View
                      </Button>
                    </TableCell>
                  </TableRow>;
              ...
              {filteredStudents.length === 0 && <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-primary-foreground">
                    No students found
                  </TableCell>
                </TableRow>}
            </TableBody>
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground pt-2">
          ...
          <p className="text-primary-foreground">Click a row to view detailed student profile</p>
        </div>
      </CardContent>
    </Card>;
};