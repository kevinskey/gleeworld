...
export function isValidIr(x: unknown): x is ExerciseIR {
  ...
  if (!Number.isFinite(ir.tonicMidi) || ir.tonicMidi < 24 || ir.tonicMidi > 96) return false;
  if (!Number.isFinite(ir.tempo) || ir.tempo <= 0) return false;
  if (!ir.meter || !Number.isFinite(ir.meter.beats) || ir.meter.beats <= 0) return false;
  if (![2, 4, 8, 16].includes(ir.meter.beatType)) return false;
  if (!Number.isFinite(ir.phrases) || !Number.isFinite(ir.difficulty)) return false;
  ...
  for (const n of ir.notes) {
    ...
    if (typeof n.solfege !== 'string' || !Number.isFinite(n.phraseIdx)) return false;
