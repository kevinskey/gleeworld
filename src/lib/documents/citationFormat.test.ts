...
// SourceForm's "Channel/Platform" field writes `publisher`; `container` is
// only a legacy fallback.
const video: DocSource = { id: '4', type: 'video',
  authors: [{ family: 'Jones', given: 'Marcus Lee' }],
  title: 'How Spirituals Became Gospel', publisher: 'YouTube',
  year: '2019', url: 'https://youtu.be/abc123' };

const videoNoAuthor: DocSource = { ...video, id: '5', authors: [] };
...
describe('MLA 9', () => {
  ...
  it('video reference quotes the title and italicizes the platform', () => {
    expect(flat(formatReference(video, 'mla9')))
      .toBe('Jones, Marcus Lee. "How Spirituals Became Gospel." YouTube, 2019, https://youtu.be/abc123.');
    expect(formatReference(video, 'mla9').filter(s => s.italic).map(s => s.text)).toEqual(['YouTube']);
  });
  it('video without an author starts at the title', () =>
    expect(flat(formatReference(videoNoAuthor, 'mla9')))
      .toBe('"How Spirituals Became Gospel." YouTube, 2019, https://youtu.be/abc123.'));
  it('video falls back to container when publisher is absent (legacy rows)', () => {
    const legacy: DocSource = { ...video, publisher: undefined, container: 'Vimeo' };
    expect(flat(formatReference(legacy, 'mla9')))
      .toBe('Jones, Marcus Lee. "How Spirituals Became Gospel." Vimeo, 2019, https://youtu.be/abc123.');
  });
  it('video with no platform still emits year and URL cleanly', () => {
    const bare: DocSource = { ...video, publisher: undefined, container: undefined };
    expect(flat(formatReference(bare, 'mla9')))
      .toBe('Jones, Marcus Lee. "How Spirituals Became Gospel." 2019, https://youtu.be/abc123.');
  });
...
describe('APA 7', () => {
  ...
  it('video reference italicizes the title and sets [Video] and the platform roman', () => {
    expect(flat(formatReference(video, 'apa7')))
      .toBe('Jones, M. L. (2019). How Spirituals Became Gospel [Video]. YouTube. https://youtu.be/abc123');
    expect(formatReference(video, 'apa7').filter(s => s.italic).map(s => s.text))
      .toEqual(['How Spirituals Became Gospel']);
  });
  it('video without an author omits the author clause', () =>
    expect(flat(formatReference(videoNoAuthor, 'apa7')))
      .toBe('(2019). How Spirituals Became Gospel [Video]. YouTube. https://youtu.be/abc123'));
  it('video falls back to container when publisher is absent (legacy rows)', () => {
    const legacy: DocSource = { ...video, publisher: undefined, container: 'Vimeo' };
    expect(flat(formatReference(legacy, 'apa7')))
      .toBe('Jones, M. L. (2019). How Spirituals Became Gospel [Video]. Vimeo. https://youtu.be/abc123');
  });
