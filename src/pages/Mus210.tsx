...
export default function Mus210() {
  ...
  const courseData = {
    ...
    objectives: ['Demonstrate proper baton and expressive gesture technique.', 'Conduct beat patterns in multiple meters and tempi.', 'Employ advanced conducting techniques.', 'Analyze choral scores for harmony, text, structure, and style.', 'Conduct rehearsals in real ensemble settings.'],
    requiredTexts: [{
      title: 'A Survey of Choral Music',
      author: 'Homer Ulrich'
    }, {
      title: 'The Modern Conductor',
      author: 'Elizabeth Green & Mark Gibson'
    }],
    ...
    gradingBreakdown: [{
      ...
      item: '5 Choral Warm-Ups (PDF)',
      percentage: 20
    }, {
      item: '30-Minute Major Work',
      percentage: 20
    }, {
      item: 'Midterm Exam',
      percentage: 15
        ...
        <div className="p-4">
          ...
          <nav className="space-y-1">
            {navItems.map(item => {
              ...
              return <button key={item.id} onClick={() => setActiveSection(item.id)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${activeSection === item.id ? 'bg-primary/10 text-primary font-medium' : 'text-foreground/80 hover:bg-muted'}`}>
            ...
            <Card className="mb-6">
              ...
              <CardContent>
                ...
                <div className="flex gap-3">
                  {isEnrolled ? <Badge variant="default" className="px-4 py-2">Enrolled</Badge> : <Button onClick={handleEnroll} disabled={isLoading}>
