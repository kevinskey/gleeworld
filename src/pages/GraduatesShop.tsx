...
export const GraduatesShop = () => {
  ...
  const loadProducts = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('is_active', true)
        .or('tags.cs.{graduates},tags.cs.{exclusive}')
        .order('title');
  ...
  const loadCartFromStorage = () => {
    const savedCart = localStorage.getItem('graduates-gleeworld-cart');
    if (savedCart) {
  ...
  const saveCartToStorage = (cart: { [key: string]: number }) => {
    localStorage.setItem('graduates-gleeworld-cart', JSON.stringify(cart));
  };
      ...
      <PublicLayout>
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading exclusive graduates products...</p>
          </div>
        </div>
      </PublicLayout>
    );
      ...
      <div className="container mx-auto px-2 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
        ...
        <div className="mb-6 p-6 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold">Graduates Exclusive Shop</h1>
          </div>
          <p className="text-sm sm:text-base text-purple-100">
            Premium merchandise and exclusive items for our distinguished graduates family
          </p>
        </div>
        ...
        <div>
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Check back soon for new graduates-exclusive merchandise!</p>
            </div>
          ) : (
