...
import { NewsletterSection } from "@/components/graduates/NewsletterSection";
import { InterviewSegments } from "@/components/graduates/InterviewSegments";
...
export default function GraduatesLanding() {
  ...
  const [graduatesStats, setGraduatesStats] = useState<GraduatesStats | null>(null);
  ...
  const [graduatesCount, setGraduatesCount] = useState<number | null>(null);

  // Check if user is graduates liaison or admin
  ...
  // Fetch global graduates count for stats
  useEffect(() => {
    const fetchGraduatesCount = async () => {
      const { count, error } = await supabase
        .from('user_roles')
        .select('*', { count: 'exact', head: true })
        .eq('role', 'graduate');
      if (!error) setGraduatesCount(count ?? 0);
    };
    fetchGraduatesCount();
  }, []);

  const fetchGraduatesStats = async () => {
    try {
      ...
      console.error('Error setting graduates stats:', error);
    } finally {
  ...
  const handleMentorToggle = async () => {
    try {
      const newMentorStatus = !graduatesStats?.mentorStatus;
      const {
        error
  ...
  const getClassYearDisplay = () => {
    if (!graduatesStats?.classYear) return "Class of ----";
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{graduatesCount ?? '—'}</p>
                </div>
