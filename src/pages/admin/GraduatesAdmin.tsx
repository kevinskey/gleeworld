...
import { HeroManager } from "@/components/graduates/HeroManager";
import { SpotlightManager } from "@/components/graduates/SpotlightManager";
import { AnnouncementManager } from "@/components/graduates/AnnouncementManager";
import { GraduatesMemberList } from "@/components/graduates/GraduatesMemberList";
...
export default function GraduatesAdmin() {
  ...
  const [graduatesProfiles, setGraduatesProfiles] = useState<GraduatesProfile[]>([]);
  ...
  const fetchAdminData = async () => {
    try {
      ...
      // Fetch graduates profiles from gw_profiles
      ...
      // Fetch users with 'graduates' role from main profiles table
      const { data: graduatesUsers } = await supabase
        .from('gw_profiles')
        .select('*')
        .eq('role', 'graduates')
        .order('created_at', { ascending: false });
      ...
      setGraduatesProfiles(combinedProfiles || []);
      setMentorStats({
  ...
  const handleVerifyAlumna = async (userId: string, verify: boolean) => {
    try {
      ...
      toast.success(`Graduate ${verify ? 'verified' : 'unverified'}!`);
      await fetchAdminData();
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{graduatesProfiles.length}</div>
              <p className="text-xs text-muted-foreground">
                {graduatesProfiles.filter(p => p.verified).length} verified
              </p>
            </CardContent>
          </Card>
        ...
        <Tabs defaultValue="registrations" className="space-y-2 md:space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs md:text-sm text-muted-foreground">
                      Manage all graduates landing page content from the tabs above
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button onClick={() => navigate('/graduates-management')} className="gap-2">
                      Page Builder
                      <Layout className="h-4 w-4" />
                    </Button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <Card className="border-2">
                    <CardContent className="pt-6">
                      ...
                      <p className="text-sm text-muted-foreground">Feature graduates interviews and stories</p>
                    </CardContent>
                  </Card>
                  <Card className="border-2">
                    <CardContent className="pt-6">
                  ...
                  <Card className="border-2">
                    <CardContent className="pt-6">
                      ...
                      <p className="text-sm text-muted-foreground">Feature outstanding graduates</p>
                    </CardContent>
                  </Card>
                  <Card className="border-2">
                    <CardContent className="pt-6">
          ...
          <TabsContent value="graduates">
            <Card>
              <CardHeader>
                <CardTitle>Graduates Verification & Management</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {graduatesProfiles.map((profile) => (
