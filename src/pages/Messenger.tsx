...
const Messenger: React.FC<MessengerProps> = ({ embedded = false, courseIdProp, courseNameProp }) => {
  ...
  // Show dropdown when filtered contacts change
  useEffect(() => {
    if (filteredContacts.length > 0) setShowDropdown(true);
  }, [filteredContacts]);
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          <tr>
            <td style="background: linear-gradient(135deg, #1e3a5f 0%, #0056a6 50%, #2563eb 100%); padding: 32px 40px; text-align: center;">
              ...
              <p style="margin: 8px 0 0 0; color: rgba(255,255,255,0.9); font-size: 14px;">${getOrgName()}</p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="background-color: #f8fafc; padding: 24px 40px; border-top: 1px solid #e2e8f0;">
              ...
              <p style="margin: 8px 0 0 0; color: #94a3b8; font-size: 12px;">© ${new Date().getFullYear()} ${getOrgName()}</p>
            </td>
          </tr>
        </table>
      </td>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-hidden">
                    ...
                    <div className="flex flex-col min-w-0 lg:border-r border-border lg:w-[65%] lg:flex-shrink-0">
                      ...
                      <div className={`flex-1 flex flex-col min-h-0 ${showEmailHistory ? 'hidden lg:flex' : 'flex'}`}>
                        <div className="flex-shrink-0 bg-card px-3 lg:px-4 pt-2 space-y-1.5">
                          {/* Recipients */}
                            ...
                            <div className="border border-border rounded-md bg-background">
                              ...
                              {recipients.length > 3 ? (
                                <Collapsible defaultOpen={false}>
                                  <CollapsibleTrigger className="flex items-center gap-2 w-full p-1.5 text-left hover:bg-muted/50 transition-colors">
                                    <ChevronRight className="h-3.5 w-3.5 transition-transform [[data-state=open]>&]:rotate-90" />
                                    <span className="text-xs font-medium">{recipients.length} recipients selected</span>
                                  </CollapsibleTrigger>
                                  <CollapsibleContent>
                                    <div className="flex flex-wrap gap-1 px-1.5 pb-1.5 max-h-[80px] overflow-y-auto">
                                      {recipients.map((r) => (
                              ...
                              <div className="relative px-1.5 pb-1.5" ref={emailDropdownRef}>
                                <Input value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setShowDropdown(true); }} onFocus={() => setShowDropdown(true)} onKeyDown={e => {
                                  ...
                                  } else if (e.key === 'Enter' && searchQuery.includes('@')) {
                          ...
                          <div className="space-y-0.5">
                            ...
                            <Input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Enter subject line..." className="h-7 bg-background border-border text-foreground text-sm placeholder:text-foreground/50" />
                          </div>
                        </div>

                        {/* Content - fills remaining space */}
                        <div className="flex-1 flex flex-col min-h-0 bg-card px-3 lg:px-4 pt-1.5 pb-2">
                        ...
                        <div className="flex-shrink-0 border-t border-border bg-card p-3 pb-[calc(env(safe-area-inset-bottom)+4.5rem)] sm:hidden">
...
...
                      {/* Send Button */}
                      <div className="flex-shrink-0 border-t border-border bg-muted p-3 pb-[calc(env(safe-area-inset-bottom)+4.5rem)]">
                        <Button onClick={handleSendSMS} disabled={isSending || (!sendToAll && smsRecipients.length === 0) || (!smsContent.trim() && smsAttachments.length === 0)} className="w-full h-9 text-sm sm:h-10 sm:text-base">
            ...
            {showGroupsPanel && <div className="w-full sm:w-72 border-l bg-muted/30 p-4 overflow-y-auto">
                ...
                {composerMode === 'email' && subject && <div className="mt-6">
                    ...
                    <div className="bg-gradient-to-br from-primary to-primary/70 rounded-t-lg p-3 text-center">
                      ...
                      <p className="text-primary-foreground/80 text-xs">{getOrgName()}</p>
                    </div>
