...
export function parseTempoBpm(input: string): TempoParse {
  ...
  if (!Number.isInteger(n)) {
    return { ok: false, message: 'Tempo must be a whole number of BPM.' };
  }
  if (n < TEMPO_BPM_MIN || n > TEMPO_BPM_MAX) {
    return { ok: false, message: `Tempo must be between ${TEMPO_BPM_MIN} and ${TEMPO_BPM_MAX} BPM.` };
  }
