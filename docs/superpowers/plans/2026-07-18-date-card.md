...
Run: `npx vitest run src/components/home/date-card/registry.test.ts`
Expected: 8 PASS. The registry is intentionally empty at this point — cards register in Tasks 3-5.
...
Expected: all PASS, including the two registration tests.
...
**Task self-consistency:** every task ends with its own tests green. Registration assertions live in T3, where cards are first registered, rather than being left failing in T1.
