...
Concert Planner's print story is fictional: the Print button is a bare `window.print()` with no `@page` size, no physical units, no pagination, and a hostile global print stylesheet (`src/index.css` ~2390: forces colors, appends raw URLs after links). The "letter-portrait / half-fold / trifold" picker only changes screen max-widths (`src/lib/concertPlanner/themes.ts:117-130` admits the paper stylesheet "handles actual geometry" — it was never written). Publish is permanently blocked (the `hasApproval` gate at `ConcertPlannerEditorPage.tsx:183` has no rendered checkbox), the QR is a hand-drawn fake, and the schema's best ideas — `sheet_music_id`, `setlist_id`, `section_heading`, `soloists`, `voicing`, `subtitle`, `notes` — have no UI.
...
| Inch-based sheet rendering + `@page size` + pagination | `WorshipAidSheets.tsx:466`, `lib/liturgy/flow.ts` | The paper renderer pattern |
...
- **`modern-clean`**: Montserrat/Inter; left-aligned pieces, composer right; thin rules instead of dot leaders.
...
- `paginateProgram(blocks, pieces, roster, designMetrics, format): Page[]` is a pure function in `src/lib/concertProgram/paginate.ts` (measures via per-block estimated heights from design metrics; DOM measurement fallback only if estimates prove insufficient during implementation — the Worship Aid flow uses metric estimates successfully).
- Print mechanism: portal overlay (PrintPaperView pattern) — `body.printing-program #root { display:none }`, `@page` style injected on mount/removed on unmount, `a[href]::after { content: none }` neutralization, then `window.print()`. Save-as-PDF in the dialog is the PDF export.

## Editor

Route `/dashboard/concert-planner/:id` (list page keeps `/dashboard/concert-planner`). Layout: true-size sheets centered on a neutral canvas (scaled to fit narrow viewports via a single `transform: scale()` wrapper — inputs unaffected), slim right rail:
...
- Publish generates slug (existing scheme) + real QR via `qrcode` → data-URL PNG shown in modal with download; optional per-program toggle "QR in footer" prints a small QR beside the footer block.
...
-- print_format: widen-then-narrow — map existing 'trifold'/'qr-lobby' rows to
-- 'letter-portrait', then replace the CHECK with ('letter-portrait','half-fold').
```

- Existing programs: a client-side `deriveDefaultBlocks(program, pieces, roster)` builds the block list on first open when `blocks = '[]'` (title, piece-groups split on stored `section_heading` changes, roster if members exist, footer) and persists it. No data backfill in SQL.
...
- Component tests (jsdom): inline edit round-trip persists via debounced patch; library picker prefills and sets `sheet_music_id`; setlist import creates group + sets `setlist_id`; publish gate needs the checkbox; QR renders from a real slug.
