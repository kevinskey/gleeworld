...
- **Calibration (new):** first launch per device runs "Clap on each of 8 flashes." Median of (clap time − pulse time) is stored as `rm_clap_latency_ms` in localStorage and shifts every hit window during play. A "recalibrate" button is always available. Rationale: Bluetooth/iPad input latency (100–300 ms) would otherwise make Perfect unattainable; follows the `takeAlignment.ts` philosophy — measure the variable part, configure the hardware part.
- **Calibration is visual-only (decided at final review, 2026-08-03).** The 8 pulses are silent on-screen flashes, not clicks, because Clap Blast is a *visually*-synchronized game: the hit line scrolls on raw musical time and the player claps to what they see, so the constant we must measure is the visual round trip (screen → hands → mic), not speaker-output + input latency. An audible metronome would over-measure by the whole output latency and grade every clap as early on Bluetooth output; it would also risk the loud clicks self-triggering the open-mic onset detector. Consequence: the stage clock stays `ctx.currentTime - t0` with no output-latency compensation anywhere.
