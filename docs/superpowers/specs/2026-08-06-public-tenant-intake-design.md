...
1. **Resolve the account.** Look up `account.email`.
   ...
   - **New account:** hold the details, do not create yet.
2. **Write the domain record.** Appointment or audition, per `kind`. If this fails
   (double-booked slot, no active audition session), return the error and stop.
3. **Provision the account** — only now, and only for the new-account case — via the admin
   API with `email_confirm: true`. Auto-confirming is deliberate: it is precisely the
   confirmation-link limbo that breaks the current flow, and the person has just proven
   control of the address by receiving the confirmation email in step 4.
4. **Notify.** Confirmation email, then welcome SMS.

Writing the domain record before creating the account means a failed booking never leaves
an orphan account behind. A failure in step 4 is logged but does not fail the submission —
the booking is real whether or not the SMS gateway is up.
...
- `book_appointment` (`supabase/migrations/20260211155714_*.sql`) is already
  ...
  `auth.uid()` — it uses `auth.uid()` only for `created_by`, which is simply NULL for anon.
  It needs `GRANT EXECUTE … TO anon` plus a guard restricting anon callers to services
...
- SMS: routed through `supabase/functions/gw-send-sms`.
...
Visitor (anon, x-tenant-slug header)
  ...
  → POST public-intake             [anon-callable edge function]
      ├─ resolve account by email  → existing? link, never modify
      ├─ write domain record       → book_appointment RPC | audition insert
      ├─ provision account         → admin API, email_confirm: true
      └─ notify                    → confirmation email + welcome SMS
...
| Slot taken between load and submit | Booking fails, nothing else runs, visitor is returned to slot selection with the taken slot removed. |
| No active audition session | Submission rejected with the existing "contact administration" message. No account created. |
...
- Unit: `public-intake` account resolution — new email, existing email, malformed email;
  assert existing accounts are never mutated.
- Unit: ordering guarantee — a forced domain-write failure creates no account.
...
- Integration: anon `book_appointment` succeeds for an active service and is refused for an
  inactive one.
