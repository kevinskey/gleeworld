...
The nginx work differed from the design in one important way, found by reading
the droplet rather than assuming.

**The rules are generated, not hand-written per vhost.** Each tenant vhost
carries `include /etc/nginx/tenant-og/<slug>.conf;` inside its `location / {`
block, and those files are produced by `/root/tenant-og.py`, which reads
`gw_tenants` joined to `gw_branding_settings`. Its header says plainly:
"Regenerated by tenant-og.py; edits here are lost on the next tenant sync." So
hand-editing the confs, as the plan described, would have been silently undone.
The change was two `sub_filter` lines added to the generator's `og_conf()`,
then one regeneration run. `sub_filter_once off` was already set.

**Favicon rules are emitted only when the tenant has a logo.** The generator
falls back to the GleeWorld og card (`og-image.png`, 1200×630) when
`logo_url` is empty. That is correct for a link preview and wrong for a 16px
tab icon, so `og_conf()` takes the raw `logo_url` and emits the icon rules only
when it is non-empty. Tenants without a logo keep the GleeWorld mark.

**Coverage is narrower than the tenant list implies.** Only 4 of 13 tenants
have a `logo_url`: `demo`, `kevin`, `lykehouse`, `campbell-hs-chorus`. Everyone
else is unchanged until they upload a logo — at which point re-running
`tenant-og.py` picks it up.

Verified server-side after reload; each tenant returned its own logo on both
icon lines, and `hhmchorus` (no logo) plus `gleeworld.org` returned the
GleeWorld default.

### Pre-existing bug found, not fixed

`tenant-og.py` builds the host as `f"{subdomain}.gleeworld.org"`, but the
`subdomain` column already holds the full host for some rows. Four tenants
therefore resolve to a doubled name and are skipped entirely:

```
demo-songwriter.gleeworld.org.gleeworld.org: NO VHOST — skipped
demo-choir.gleeworld.org.gleeworld.org:      NO VHOST — skipped
demo-school.gleeworld.org.gleeworld.org:     NO VHOST — skipped
demo-district.gleeworld.org.gleeworld.org:   NO VHOST — skipped
```

Those four get neither per-tenant link previews nor favicons. This predates
this work and is the same full-host-in-`subdomain` trap already documented for
the nav sidebar's tenant cards. Fixing it means changing the host derivation
and re-running; out of scope here, but it should be fixed — those tenants have
been previewing as "GleeWorld" the whole time.

### Known risk

`/root/tenant-og.py` exists **only on the droplet**. It is not in this repo, has
no version history, and no backup outside the box. Every tenant's link previews
and favicons depend on it. A pre-change backup was taken to
`/root/nginx-backup-20260806-1545.tar.gz` and `/root/tenant-og.py.bak-*`.
